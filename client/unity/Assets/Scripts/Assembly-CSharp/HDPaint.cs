...
public class HDPaint : IPaint
{
	...
	static HDPaint()
	{
		...
		TField.xDu = 8;
		TField.yDu = 8;
		PaintPopup.color = new int[6] { 3521446, 2378578, 8052436, 2716523, 16701696, 7042560 };
		...
		imgEraser = new FrameImage(Image.createImagePNG(T.getPath() + "/temp/eraser"), 26, 26);
		imgMSG = new Image[2];
		...
		imgCheck = new FrameImage(Image.createImagePNG(T.getPath() + "/effect/check"), 43, 43);
		...
		for (int j = 0; j < 2; j++)
		{
			...
			for (int k = 0; k < 4; k++)
			{
				PaintPopup.imgMuiIOS[j][k] = Image.createImagePNG(T.getPath() + "/ios/a" + j + string.Empty + k);
			}
		}
		MsgDlg.hCell = PaintPopup.imgMuiIOS[0][2].getHeight() + 8;
		imgTrans = Image.createImage(T.getPath() + "/effect/trans");
		AvMain.hCmd = 58;
		imgPopupBack = new FrameImage(Image.createImagePNG(T.getPath() + "/effect/popupBack"), 40, 40);
		imgPopupBackNum = new Image[4];
		for (int l = 0; l < 4; l++)
		{
			imgPopupBackNum[l] = Image.createImagePNG(T.getPath() + "/effect/popupBack" + l);
		}
		imgEffectBack = new FrameImage(Image.createImagePNG(T.getPath() + "/effect/effectPopupBack"), 100, 50);
		imgFocusTab = new FrameImage(Image.createImagePNG(T.getPath() + "/iconMenu/tabnotfocus"), 24, 75);
		imgNotFocusTab = new FrameImage(Image.createImagePNG(T.getPath() + "/iconMenu/tabfocus"), 24, 61);
	...
	public void paintTextBox(MyGraphics g, int x, int y, int width, int height, TField tf, bool isFocus, sbyte indexEraser)
	{
		...
		g.setColor(2720192);
		g.fillRect(x + TField.tfframe.frameWidth, y, width - TField.tfframe.frameWidth * 2, 2f);
		g.fillRect(x + TField.tfframe.frameWidth, y + height - 2, width - TField.tfframe.frameWidth * 2, 2f);
		g.setColor(16777215);
		g.fillRect(x + TField.tfframe.frameWidth, y + 2, width - TField.tfframe.frameWidth * 2, height - 4);
		if (tf.isFocused() && !tf.paintedText.Equals(string.Empty))
		{
			imgEraser.drawFrame(indexEraser, x + width - 20, y + height / 2, 0, 3, g);
		}
		g.setClip(x + 3, y + 1, width - 8, height - 4);
		g.setColor(0);
		...
		{
			g.setColor(0);
			g.fillRect(TField.TEXT_GAP_X + tf.offsetX + x + Canvas.fontChatB.getWidth(tf.paintedText.Substring(0, tf.caretPos) + "a") - Canvas.fontChatB.getWidth("a") - 1 + 6, y + 12, 1f, height - 24);
		}
	}
	...
	public void paintPopupBack(MyGraphics g, int x, int y, int w, int h, int countClose, bool isFull)
	{
		...
		if (countClose != -1)
		{
			if (!isFull)
			{
				g.drawImage(ListScr.imgCloseTab[countClose], x + w - 3, y, 3);
			}
			else
			{
	...
	public void paintBoxTab(MyGraphics g, int x, int y, int h, int w, int focusTab, int wSub, int wTab, int hTab, int numTab, int maxTab, int[] count, int[] colorTab, string name, sbyte countCloseAll, sbyte countText, bool isMenu, bool isFull, string[] subName, float cmx, Image[][] imgIcon)
	{
		...
		imgFocusTab.drawFrame(0, xTab + x + focusTab * wTabDu + wTabDu / 2 - wTab / 2, y + hTab - imgFocusTab.frameHeight + 6, 0, g);
		imgFocusTab.drawFrame(1, xTab + x + focusTab * wTabDu + wTabDu / 2 + wTab / 2 - imgFocusTab.frameWidth, y + hTab - imgFocusTab.frameHeight + 6, 0, g);
		g.drawImageScale(imgFocusTab_1, xTab + x + focusTab * wTabDu + wTabDu / 2 - wTab / 2 + imgFocusTab.frameWidth, y + hTab - imgFocusTab.frameHeight + 6, wTab - imgFocusTab.frameWidth * 2, imgFocusTab_1.getHeight(), 0);
		if (numTab > 1)
		{
			if (imgIcon != null)
			{
		...
		{
			g.setClip(xTab + x + focusTab * wTabDu + wTabDu / 2 - wTab / 2 + 4, y + hTab - imgFocusTab.frameHeight + 6, wTab - 8, hTab);
		...
		if (countCloseAll != -1)
		{
			if (!isFull)
			{
				g.drawImage(ListScr.imgCloseTab[countCloseAll], x + w + 5 - 5 * AvMain.hd, y + hTab + 2, 3);
			}
			else
			{
				g.drawImage(ListScr.imgCloseTabFull[countCloseAll], x + w - 10 - (isFull ? 25 : 0), y + hTab + 10 - (isFull ? 23 : 0), 3);
			}
		}
	}
	...
	public void paintCheckBox(MyGraphics g, int x, int y, int focus, bool isCheck)
	{
		...
		Canvas.tempFont.drawString(g, T.rememPass, x + 50, y + imgCheck.frameHeight / 2 - 4, 0);
	}
	...
	public void paintArrow(MyGraphics g, int index, int x, int y, int w, int indLeft, int indRight)
	{
		...
		g.drawImage(PaintPopup.imgMuiIOS[indRight][3], x + w, y, 3);
	}
	...
	{
		Canvas.resetTrans(g);
		g.drawImage(PaintPopup.imgMuiIOS[ind1][2], x - 80, y, 3);
		g.drawImage(PaintPopup.imgMuiIOS[ind0][3], x + 80, y, 3);
		g.drawImage(PaintPopup.imgMuiIOS[ind3][0], x, y - 80, 3);
		g.drawImage(PaintPopup.imgMuiIOS[ind2][1], x, y + 80, 3);
	}
	...
	public bool selectedPointer(int xF, int yF)
	{
		if (xF > 0 && yF > 0 && CRes.distance(xF / AvMain.hd, yF / AvMain.hd, GameMidlet.avatar.x, GameMidlet.avatar.y) <= ((Canvas.currentMyScreen != FarmScr.instance) ? 70 : (300 * AvMain.hd)))
		{
			...
			for (int i = 0; i < LoadMap.playerLists.size(); i++)
			{
				...
				if (@base.IDDB != GameMidlet.avatar.IDDB && @base.catagory != 4 && ((@base.IDDB == GameMidlet.avatar.IDDB && Canvas.currentMyScreen != FarmScr.instance) || @base.IDDB != GameMidlet.avatar.IDDB) && CRes.abs(@base.x * AvMain.hd - xF) <= 40 && @base.y * AvMain.hd - yF < 60 && @base.y * AvMain.hd - yF > 0)
				{
	...
	public void paintMSG(MyGraphics g)
	{
		...
		if (Canvas.setShowIconMenu())
		{
			...
			{
				num = 140;
				g.drawImage(iconMenu[indexMenu], 65f, 35 + Canvas.countTab, 3);
			}
			else
			{
				num = 100;
				g.drawImage(iconMenu_2[indexMenu], 45f, 41 + Canvas.countTab, 3);
			}
		}
		if (Canvas.setShowMsg())
		{
			g.drawImage(imgMSG[indexMSG], num + imgMSG[0].w / 2, 40 + Canvas.countTab, 3);
			if (MessageScr.gI().isNewMsg)
			{
				g.drawImage(imgNewMsg, num + imgMSG[0].w / 2 + imgMSG[indexMSG].w / 2 - 6, 40 + Canvas.countTab + imgMSG[indexMSG].h / 2 - 6, 3);
			}
		}
		...
		g.drawImage(MyScreen.imgChat[indexChat], num + imgMSG[0].w + 45, 40 + Canvas.countTab, 3);
		if (Canvas.currentMyScreen != RaceScr.me && !Bus.isRun)
		{
			g.drawImage(iconRota[indexRota], Canvas.w - 40 - 100, 40 + Canvas.countTab, 3);
		}
		if (!onMainMenu.isOngame && Canvas.currentMyScreen != RaceScr.me && (Canvas.menuMain == null || Canvas.menuMain != MenuIcon.me))
		{
			...
			{
				g.drawImage(iconAction[indexAction], Canvas.w - 40, 40 + Canvas.countTab, 3);
			}
			g.drawImage(iconFeel[indexFeel], Canvas.w - 40, 120 + Canvas.countTab, 3);
		}
	}
	...
	public void setBack()
	{
		...
		if (Canvas.setShowIconMenu())
		{
			num = ((Canvas.currentMyScreen != MapScr.instance) ? 100 : 140);
		}
		if (Canvas.isPointerClick)
		{
			isTranIcon = true;
			...
			else if (Canvas.isPaintIconVir() && Canvas.isPoint(num + imgMSG[0].w + 6, 0, 77, 80 + Canvas.countTab))
			{
				indexChat = 1;
			}
			else if (Canvas.isPaintIconVir() && GameMidlet.avatar.action != 14 && !onMainMenu.isOngame && Canvas.currentMyScreen != RaceScr.me && (Canvas.menuMain == null || Canvas.menuMain != MenuIcon.me) && Canvas.isPoint(Canvas.w - 80, Canvas.countTab, 80, 80))
			{
				indexAction = 1;
			}
		...
		if (Canvas.isPointerDown)
		{
			if (indexMenu == 1 && Canvas.currentMyScreen != MapScr.instance && !Canvas.isPoint(0, 0, 90, 70 + Canvas.countTab))
			{
				indexMenu = 0;
			}
			if (indexMenu == 1 && Canvas.currentMyScreen == MapScr.instance && !Canvas.isPoint(0, 0, 130, 70 + Canvas.countTab))
			{
				indexMenu = 0;
			}
			else if (indexMSG == 1 && !Canvas.isPoint(num - 10, 0, 80, 70 + Canvas.countTab))
			{
				indexMSG = 0;
			}
			else if (indexChat == 1 && !Canvas.isPoint(num + imgMSG[0].w + 6, 0, 77, 70 + Canvas.countTab))
			{
				indexChat = 0;
			}
			else if (indexAction == 1 && !Canvas.isPoint(Canvas.w - 80, Canvas.countTab, 80, 80))
			{
				indexAction = 0;
			}
			else if (indexFeel == 1 && !Canvas.isPoint(Canvas.w - 80, 80 + Canvas.countTab, 80, 80))
			{
				indexFeel = 0;
			}
			else if (indexRota == 1 && !Canvas.isPoint(Canvas.w - 40 - 150, 40 + Canvas.countTab - 50, 100, 100))
			{
				indexRota = 0;
			}
		}
	...
	public void setDrawPointer(Command left, Command center, Command right)
	{
		...
		if (isTranKeyPointer && Canvas.isPointerDown)
		{
			int num = collisionCmdBar(left, center, right);
			if (AvMain.indexLeft != 0 && num != 0)
			{
				AvMain.indexLeft = 0;
			}
			if (AvMain.indexCenter != 0 && num != 1)
			{
				AvMain.indexCenter = 0;
			}
			if (AvMain.indexRight != 0 && num != 2)
			{
				AvMain.indexRight = 0;
			}
		}
	...
	public void initImgBoard(int type)
	{
		...
		BoardListOnScr.imgBoard = new FrameImage(Image.createImagePNG("hd/hd/on/" + empty), 162, 128);
	...
	public void initOngame()
	{
		try
		{
			...
			imgButtonOn = new FrameImage(Image.createImagePNG("hd/hd/on/imgButton"), 168, 56);
			...
			imgBar = Image.createImagePNG("hd/hd/on/imgBar");
			PaintPopup.hButtonSmall = imgBar.h;
			imgBarMoney = Image.createImagePNG("hd/hd/on/barMoney");
		}
		catch (Exception e)
		{
			Out.logError(e);
	...
	{
		g.setColor(5299141);
		g.fillRect(8f, PaintPopup.hTab + 90, PaintPopup.gI().w - 16, 80f);
		imgRegGender.drawFrame((male == 2) ? 1 : 0, PaintPopup.gI().w / 2 - 41, PaintPopup.hTab + 45, 0, 3, g);
		imgRegGender.drawFrame((male != 2) ? 1 : 0, PaintPopup.gI().w / 2 + 41, PaintPopup.hTab + 45, 0, 3, g);
		Canvas.blackF.drawString(g, T.gender[0], PaintPopup.gI().w / 2 - 42, PaintPopup.hTab + 45 - AvMain.hBlack / 2, 2);
		Canvas.blackF.drawString(g, T.gender[1], PaintPopup.gI().w / 2 + 42, PaintPopup.hTab + 45 - AvMain.hBlack / 2, 2);
		g.drawImage(PaintPopup.imgMuiIOS[countLeft / 3][2], PaintPopup.gI().w / 2 - 90, PaintPopup.hTab + 130, 3);
		g.drawImage(PaintPopup.imgMuiIOS[countRight / 3][3], PaintPopup.gI().w / 2 + 90, PaintPopup.hTab + 130, 3);
		GameMidlet.avatar.paintIcon(g, PaintPopup.gI().w / 2 + 1, PaintPopup.hTab + 160, false);
		Canvas.resetTrans(g);
	}
	...
	public void updateKeyRegister()
	{
		if (Canvas.isPointerClick)
		{
			if (Canvas.isPoint(PaintPopup.gI().x + PaintPopup.gI().w / 2 - 41 - 40, PaintPopup.gI().y + PaintPopup.hTab + 45 - 30, 80, 60))
			{
				RegisterScr.gI().male = 1;
				RegisterScr.gI().getAvatarPart();
				Canvas.isPointerClick = false;
			}
			else if (Canvas.isPoint(PaintPopup.gI().x + PaintPopup.gI().w / 2 + 41 - 40, PaintPopup.gI().y + PaintPopup.hTab + 45 - 30, 80, 60))
			{
				RegisterScr.gI().male = 2;
				RegisterScr.gI().getAvatarPart();
				Canvas.isPointerClick = false;
			}
			else if (Canvas.isPoint(PaintPopup.gI().x + PaintPopup.gI().w / 2 - 130, PaintPopup.gI().y + PaintPopup.hTab + 90, 80, 80))
			{
			...
			else if (Canvas.isPoint(PaintPopup.gI().x + PaintPopup.gI().w / 2 + 50, PaintPopup.gI().y + PaintPopup.hTab + 90, 80, 80))
			{
				RegisterScr.gI().index = 1;
				RegisterScr.gI().setKeyLeftRight(1);
	...
	public void initReg()
	{
		try
		{
			imgRegGender = new FrameImage(Image.createImage(T.getPath() + "/gender"), 67, 40);
		}
		catch (Exception e)
		{
			Out.logError(e);
	...
	public void paintMoney(MyGraphics g, int x, int y)
	{
		...
		g.drawImage(MyInfoScr.gI().imgIcon[0], x + width + 38, y + Canvas.tempFont.getHeight() / 2, 3);
		g.drawImage(MyInfoScr.gI().imgIcon[1], x + width2 + 140 + width, y + Canvas.tempFont.getHeight() / 2, 3);
	...
	public void paintTabSoft(MyGraphics g)
	{
		...
		g.drawImageScale(imgBar, 0, Canvas.hCan - height, Canvas.w, height, 0);
	}
	...
	public void paintCmdBar(MyGraphics g, Command left, Command center, Command right)
	{
		...
		if (left != null && left.caption != string.Empty)
		{
			if (left.x != -1)
			{
				imgButtonOn.drawFrame(indexLeft, left.x + PaintPopup.wButtonSmall / 2, num, 0, 3, g);
		...
		if (center != null && center.caption != string.Empty)
		{
			if (center.x != -1)
			{
				imgButtonOn.drawFrame(indexCenter, center.x + PaintPopup.wButtonSmall / 2, num, 0, 3, g);
		...
		if (right != null && right.caption != string.Empty)
		{
			if (right.x != -1)
			{
				imgButtonOn.drawFrame(indexRight, right.x + PaintPopup.wButtonSmall / 2, num, 0, 3, g);
	...
	{
		g.drawImage(imgBarMoney, x + w / 2, y + h / 2 + 4, 3);
	}
}
